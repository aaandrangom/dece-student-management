...
import {
    ...
    FileSignature, Gavel, Eye, ArrowLeft, Calendar, User
} from 'lucide-react';
...
export default function LlamadosAtencion({ matriculaId, nombreEstudiante, onBack }) {
    ...
    return (
        <div className="bg-slate-50/50 min-h-full font-sans animate-in fade-in duration-300">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-40 p-4 transition-all animate-in fade-in zoom-in-95 duration-200">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
                            <div>
                                <h3 className="text-xl font-bold text-slate-800">{formData.id === 0 ? 'Registrar Falta' : 'Editar Registro'}</h3>
                                <p className="text-xs text-slate-500">Complete la información del llamado de atención.</p>
                            </div>
                            ...
                            {isLoading ? (
                                ...
                                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-6">
                                    ...
                                    <div className="md:col-span-12">
                                        <h4 className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-4 border-b border-blue-100 pb-1">Detalle de la Falta</h4>
                                    </div>

                                    <div className="md:col-span-4">
                                        <label className="block text-sm font-semibold text-slate-700 mb-1.5 items-center gap-2">
                                            <Calendar className="w-4 h-4 text-slate-400" /> Fecha de Suceso
                                        </label>
                                        <input type="date" required value={formData.fecha} onChange={(e) => updateField('fecha', e.target.value)}
                                            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                                    </div>

                                    <div className="md:col-span-8">
                                        <label className="block text-sm font-semibold text-slate-700 mb-1.5">Descripción de la Falta</label>
                                        <input type="text" required placeholder="Ej: Uso inadecuado de uniforme..." value={formData.motivo} onChange={(e) => updateField('motivo', e.target.value)}
                                            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                                    </div>

                                    <div className="md:col-span-6 flex flex-col h-full bg-slate-50 p-5 rounded-xl border border-slate-100">
                                        <h4 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                                            <FileSignature className="w-4 h-4 text-slate-500" /> Proceso Administrativo
                                        </h4>
                                        <div className="space-y-4 flex-1">
                                            <label className="flex items-center gap-3 p-2 hover:bg-white rounded-lg transition-colors cursor-pointer border border-transparent hover:border-slate-200">
                                                <input type="checkbox" checked={formData.representante_notificado} onChange={(e) => updateField('representante_notificado', e.target.checked)} className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 border-gray-300" />
                                                <span className="text-sm text-slate-700 font-medium">Representante Notificado</span>
                                            </label>

                                            <div className="space-y-2">
                                                <label className="flex items-center gap-3 p-2 hover:bg-white rounded-lg transition-colors cursor-pointer border border-transparent hover:border-slate-200">
                                                    <input type="checkbox" checked={formData.representante_firmo} onChange={(e) => updateField('representante_firmo', e.target.checked)} className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 border-gray-300" />
                                                    <span className="text-sm text-slate-700 font-medium">Representante Firmó Acta</span>
                                                </label>
                                                {!formData.representante_firmo && (
                                                    <div className="pl-9 animate-in slide-in-from-top-2 duration-200">
                                                        <input type="text" placeholder="Motivo por el que no firmó..." value={formData.motivo_no_firma} onChange={(e) => updateField('motivo_no_firma', e.target.value)}
                                    ...
                                    <div className="md:col-span-6 flex flex-col h-full bg-slate-50 p-5 rounded-xl border border-slate-100">
                                        <h4 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
                                            <Gavel className="w-4 h-4 text-slate-500" /> Resolución / Sanción
                                        </h4>
                                        <div className="space-y-3 flex-1 flex flex-col">
                                            <textarea rows="3" placeholder="Describa la medida disciplinaria..." value={formData.medida_disciplinaria} onChange={(e) => updateField('medida_disciplinaria', e.target.value)}
                                                className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg outline-none focus:border-blue-500 text-sm resize-none"></textarea>

                                            <div className="mt-auto pt-2 border-t border-slate-200/50">
                                                <div className="flex items-center gap-4 mb-2">
                                                    <label className="flex items-center gap-2 cursor-pointer">
                                                        <div className={`w-10 h-6 rounded-full p-1 transition-colors duration-300 ${formData.cumplio_medida ? 'bg-green-500' : 'bg-slate-300'}`}>
                                                            <div className={`bg-white w-4 h-4 rounded-full shadow-sm transform transition-transform duration-300 ${formData.cumplio_medida ? 'translate-x-4' : ''}`}></div>
                                                        </div>
                                                        <input type="checkbox" className="hidden" checked={formData.cumplio_medida} onChange={(e) => updateField('cumplio_medida', e.target.checked)} />
                                                        <span className="text-sm font-bold text-slate-700">¿Cumplió medida?</span>
                                                    </label>
                                                </div>
                                                {!formData.cumplio_medida && (
                                                    <input type="text" placeholder="Detalle incumplimiento..." value={formData.motivo_incumplimiento} onChange={(e) => updateField('motivo_incumplimiento', e.target.value)}
                                                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:border-red-400 outline-none" />
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="md:col-span-12 pt-2">
                                        <h4 className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-4 border-b border-blue-100 pb-1">Documentación Digital</h4>

                                        {formData.id === 0 ? (
                                            <div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-4 rounded-xl text-sm flex items-center justify-center gap-3">
                                                <AlertTriangle className="w-5 h-5" />
                                                <span className="font-medium">Debe guardar el registro primero para adjuntar evidencia digital.</span>
                                            </div>
                                        ) : (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {[
                                                    { key: 'ruta_acta', label: 'Acta de Compromiso', icon: FileSignature, color: 'green', type: 'acta' },
                                                    { key: 'ruta_resolucion', label: 'Resolución Rectoral', icon: Gavel, color: 'purple', type: 'resolucion' }
                                                ].map((doc) => (
                                                    <div key={doc.type} className="border border-slate-200 rounded-xl p-4 flex items-center justify-between bg-white hover:shadow-md transition-shadow">
                                                        <div className="flex items-center gap-4">
                                                            <div className={`p-2.5 rounded-lg ${formData[doc.key] ? `bg-${doc.color}-100 text-${doc.color}-600` : 'bg-slate-100 text-slate-400'}`}>
                                                                <doc.icon className="w-6 h-6" />
                                                            </div>
                                                            <div>
                                                                <p className="font-bold text-sm text-slate-800">{doc.label}</p>
                                                                <p className={`text-xs ${formData[doc.key] ? 'text-green-600 font-medium' : 'text-slate-400'}`}>
                                                                    {formData[doc.key] ? 'Documento cargado' : 'Pendiente de carga'}
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            {formData[doc.key] && (
                                                                <button type="button" onClick={() => handlePreview(formData[doc.key])} className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg border border-slate-200 transition-all" title="Ver">
                                                                    <Eye className="w-4 h-4" />
                                                                </button>
                                                            )}
                                                            <button type="button" onClick={() => handleUpload(formData.id, doc.type)} className={`px-3 py-2 rounded-lg border text-xs font-bold transition-all ${doc.color === 'green' ? 'text-green-700 bg-green-50 border-green-200 hover:bg-green-100' : 'text-purple-700 bg-purple-50 border-purple-200 hover:bg-purple-100'}`}>
                                                                {formData[doc.key] ? 'Cambiar' : 'Subir PDF'}
                                                            </button>
                                                        </div>
                                                    </div>
                                                ))}
                                    ...
                                    <div className="md:col-span-12 flex justify-end gap-3 pt-6 mt-2 border-t border-slate-100">
                                        <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 font-medium transition-colors">Cancelar</button>
                                        <button type="submit" disabled={isSubmitting} className="px-8 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium flex items-center gap-2 shadow-sm hover:shadow-md transition-all disabled:opacity-70 disabled:cursor-not-allowed">
