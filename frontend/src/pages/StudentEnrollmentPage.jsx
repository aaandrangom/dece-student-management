...
const StudentEnrollmentPage = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        representante_rol: 'PADRE', // PADRE, MADRE, OTRO
    ...
    // Cargar aulas cuando cambia el año
    useEffect(() => {
        if (formData.anio_id) {
    ...
    // Actualizar aula_id cuando cambian curso o paralelo
    useEffect(() => {
        ...
        try {
            ...
            if (student) {
                ...
                setFormData(prev => ({
                    ...prev,
                    ...student,
                    // Mantener campos académicos limpios para la nueva matrícula
                    anio_id: prev.anio_id,
                    curso_id: '',
                    paralelo_id: '',
                    aula_id: 0,
                    es_repetidor: false,
                    institucion_procedencia: student.institucion_procedencia || '' // Mantener procedencia si existe
                }));
            ...
            // Si da error (ej: record not found), asumimos que es nuevo
            setIsExistingStudent(false);
    ...
    const renderStep3 = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-indigo-50 rounded-xl p-6 border border-indigo-100">
                <label className="block text-xs font-bold text-indigo-800 uppercase mb-3 flex items-center gap-2">
                    ...
                    {['PADRE', 'MADRE', 'OTRO'].map(rol => (
                        <label 
                            ...
                            className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border cursor-pointer transition-all ${
                                formData.representante_rol === rol 
                        ...
                        {[
                            ...
                            return (
                                <div key={s.id} className="flex flex-col items-center gap-2">
                                    <div 
                                        className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 border-4 border-white ${
                                            isActive 
